...
<div class="ml-5 mt-3">
	<img src="/resources/profileImage/${otherUser.profileImage }" class="rounded-circle myimage mr-3" alt="Cinque Terre">
	<span style="font-size: large; font-weight: bold;">${otherUser.nickname }</span>
	<div class="text-center">
		<button class="btn btn-outline-danger btn-sm">신고하기
			<svg width="2em" height="2em" viewBox="0 0 16 16" class="bi bi-brightness-alt-high" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
		...
		<button class="btn btn-outline-primary btn-sm"> 쪽지 보내기
			<svg width="2em" height="2em" viewBox="0 0 16 16" class="bi bi-mailbox2" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
		...
		<button class="btn btn-outline-light text-dark btn-sm"> 팔로우하기
		...
		<button class="btn btn-outline-light text-dark btn-sm"> 취소
	...
	<div class="col-12">
		<nav class="navbar">
			<ul class="nav nav-tabs" id="otherpage-nav">
				<li class="nav-item"><a class="nav-link active" href="#" id="otherpage-info">정보</a>
				</li>
				<li class="nav-item"><a class="nav-link" href="#" id="otherpage-mymoim">가입모임</a>
				</li>
				<li class="nav-item"><a class="nav-link" href="#" id="otherpage-images">사진첩</a></li>
				<li class="nav-item"><a class="nav-link" href="#" id="otherpage-board">작성후기</a></li>
			</ul>
		</nav>
	</div>
</div>
...
<script>
	...
	$(function () {
		...
		var otherpageInfo =
			...
			otherpageInfo +=
			"<tr><th class='text-center'>아이디</th><td>${otherUser.id}</td><th class='text-center'>전화번호</th><td>${otherUser.tel}</td></tr>";
		...
		$("#otherpage-images").click(function () {
			$.ajax({
				type: "GET",
				url: "/other/photo.do",
				dataType: "json",
				success: function (images) {
					if (images.status == 'false') {
						...
						$.each(images.photos, function (index, image) {
		...
		$("#otherpage-board").click(function () {
			$.ajax({
				type: "GET",
				url: "/other/board.do",
				dataType: "json",
				success: function (boards) {
					...
					if (boards.status == 'false') {
						...
						$.each(boards.boards, function (index, board) {
							var tr = "<div class='card'>";
							tr += "<a href='/moim/boardDetail.do?boardNo=" + board.boardNo + "'>";
							tr +=
								"<img class='card-img-top' src='/resources/home_images/9.jpg' alt='Card image cap'>";
							tr += "<div class='card-body'>";
							tr += "<h5 class='card-title'>" + board.title + "</h5>"
							tr += "<p class='card-text'>" + board.content + "</p>";
							tr += "</div><div class='card-footer'>";
							tr += "<small class='text-muted'>" + board.createdDate + "</small>";
							tr += "</div></a></div>";
